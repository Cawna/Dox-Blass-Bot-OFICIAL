            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    let apii = await this.getFile(pp)
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'EL GRUPO NO TIENE DESCRIPCIÓN.') :
...
export async function callUpdate(callUpdate) {
    ...
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦;;;\nFN:© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦\nORG:© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦\nTITLE:\nitem1.TEL;waid=51946352266:+51 946 352 266\nitem1.X-ABLabel:© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: 'Comando solo para el uso del creador.',
        owner: 'Comando solo para el uso del creador.',
        mods: 'Comandó solo para moderadores.',
        premium: ' 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾𝙻𝙾 𝙿𝚄𝙴𝙳𝙴 𝚂𝙴𝚁 𝚄𝚃𝙸𝙻𝙸𝚉𝙰𝙳𝙾 𝙿𝙾𝚁 𝚄𝚂𝚄𝙰𝚁𝙸𝙾𝚂 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 𝚈 𝙴𝙻/𝙻𝙰 𝙿𝚁𝙾𝙿𝙸𝙴𝚃𝙰𝚁𝙸𝙾/𝙰 (𝙾𝚆𝙽𝙴𝚁) 𝙳𝙴𝙻 𝙱𝙾𝚃*',
        group: 'Solo lo puedes usar en grupos.',
        private: 'Solo lo puedes usar en privado',
        admin: 'Solo los administradores pueden usar este comando.',
        botAdmin: 'Primeramente tienes que darme administración para proceder con eso.',
        unreg: '*[ 🛑 𝐇𝐄𝐘!! 𝐀𝐋𝐓𝐎, 𝐍𝐎 𝐄𝐒𝐓𝐀𝐒 𝐑𝐄𝐆𝐈𝐒𝐓𝐑𝐀𝐃𝐎 🛑 ]*\n\n*—◉ 𝙿𝙰𝚁𝙰 𝚄𝚂𝙰𝚁 𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙳𝙴𝙱𝙴𝚂 𝚁𝙴𝙶𝙸𝚂𝚃𝚁𝙰𝚁𝚃𝙴, 𝚄𝚂𝙰 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾*\n*➣ #verificar nombre.edad*',
        restrict: 'Solo para creadores.'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '📱 AVISÓ - ALERTA 📱', body: '© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧', thumbnail: imagen1, sourceUrl: 'https://api.whatsapp.com/send?phone=+51946352266' }}}}, aa)
