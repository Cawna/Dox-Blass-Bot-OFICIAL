...
if (!text) throw `¿Qué estas buscando?`
try {
if (command == 'play.mp3') {
conn.reply(m.chat, `Se está enviando su audio.`, m) 
try {  
...
if (command == 'play.mp4') {
conn.reply(m.chat, `Se está enviando su video.`, m)    
try {   
let mediaa = await ytPlayVid(text)
await conn.sendMessage(m.chat, { video: { url: mediaa.result }, fileName: `error.mp4`, caption: `© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧`, thumbnail: mediaa.thumb, mimetype: 'video/mp4' }, { quoted: m })   
} catch {
let res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`)    
let json = await res.json()
await conn.sendFile(m.chat, json.result.video, 'error.mp4', `© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧`, m)}}
} catch (e) {
m.reply('Error, vuelve a intentarlo.')
}}
handler.help = ['play.mp3' , 'play.mp4'].map(v => v + ' <texto>')
handler.tags = ['downloader']
handler.command = ['play.mp3', 'play.mp4']
