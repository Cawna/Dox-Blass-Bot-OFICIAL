...
function purgeSession() {
    let prekey = []
    let directorio = readdirSync("./DoxQR")
    let filesFolderPreKeys = directorio.filter(file => {
        return file.startsWith('pre-key-')
    })
    prekey = [...prekey, ...filesFolderPreKeys]
    filesFolderPreKeys.forEach(files => {
    unlinkSync(`./DoxQR/${files}`)
})
...
const directories = ['./DoxQR/', './jadibts/']
...
console.log(chalk.yellow('📱Escanea este codigo QR, el codigo QR expira en 30 segundos.'))
}
if (connection == 'open') {
console.log(chalk.yellow('Conectado correctamente con tu WhatsApp.'))}
if (connection == 'close') {
...
conn.welcome = '© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧\n\n📱BIENVENIDA AUTOMÁTICA\n\nUsuario: @user\nGrupo: @subject\nDescripción:\n@desc'
conn.bye = '© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧\n\n📱DESPEDIDA AUTOMÁTICA \n\nUsuario: @user\nEl usuario acaba de salir o fué eliminado.'
conn.spromote = '@user AHORA ES UN ADMINISTRADOR.'
conn.sdemote = '@user YA NO FORMA PARTE FE LOS ADMINISTRADORES.'
conn.sDesc = '@desc'
conn.sSubject = '@subject'
conn.sIcon = 'SÉ CAMBIO EL ICONO DEL GRUPO.'
conn.sRevoke = '@revoke'
...
let bio = `© 𝗗𝝮̶𝗫 𝗕𝗟𝗔𝗦𝗦 𝗕𝝮̶𝗧 ┃${uptime} `
